...
/**
 * Copied from {@link org.gradle.testkit.runner.internal.DefaultGradleRunner}, additionally allows you to determine
 * the executor at runtime
 */
public abstract class AbstractGradleRunner extends GradleRunner {
    ...
    protected AbstractGradleRunner() {
    ...
    protected abstract GradleExecutor getNextExecutor();

    static class ConstantTestKitDirProvider implements TestKitDirProvider {
    ...
    protected static TestKitDirProvider calculateTestKitDirProvider(SystemProperties systemProperties) {
    ...
    @Override
    public AbstractGradleRunner withTestKitDir(File testKitDir) {
    ...
    public AbstractGradleRunner withJvmArguments(List<String> jvmArguments) {
        this.jvmArguments = Collections.unmodifiableList(new ArrayList<>(jvmArguments));
        return this;
    }

    public AbstractGradleRunner withJvmArguments(String... jvmArguments) {
    ...
    @Override
    public AbstractGradleRunner withProjectDir(File projectDir) {
    ...
    @Override
    public AbstractGradleRunner withArguments(List<String> arguments) {
        this.arguments = Collections.unmodifiableList(new ArrayList<>(arguments));
        return this;
    }

    @Override
    public AbstractGradleRunner withArguments(String... arguments) {
    ...
    private BuildResult run(Action<GradleExecutionResult> resultVerification) {
        ...
        if (environment != null && debug) {
            throw new InvalidRunnerConfigurationException(
                    "Debug mode is not allowed when environment variables are specified. Debug mode runs 'in process'"
                            + " but we need to fork a separate process to pass environment variables. To run with debug"
                            + " mode, please remove environment variables.");
        }
        ...
        GradleExecutionResult execResult = getNextExecutor()
                .run(new GradleExecutionParameters(
    ...
    private static BuildResult createBuildResult(GradleExecutionResult execResult) {
        return new FeatureCheckBuildResult(
                execResult.getBuildOperationParameters(), execResult.getOutput(), execResult.getTasks());
    }
