...
public class NamedToolingApiGradleExecutor implements GradleExecutor {

    private final String daemonDirName;

    public NamedToolingApiGradleExecutor(String daemonDirName) {
        this.daemonDirName = daemonDirName;
    }
    ...
    @SuppressWarnings({"CatchAndPrintStackTrace", "ShutdownHook"}) // We don't have logging infrastructure here
    ...
    public GradleExecutionResult run(GradleExecutionParameters parameters) {
        ...
        try {
            ...
            if (targetGradleVersion.compareTo(TestKitFeature.RUN_BUILDS.getSince()) < 0) {
                throw new UnsupportedFeatureException(String.format(
                        "The version of Gradle you are using (%s) is not supported by TestKit. TestKit supports all"
                                + " Gradle versions %s and later.",
            ...
            return new GradleExecutionResult(
                    new BuildOperationParameters(targetGradleVersion, parameters.isEmbedded()), output, tasks, t);
        ...
        return new GradleExecutionResult(
                new BuildOperationParameters(targetGradleVersion, parameters.isEmbedded()), output, tasks);
    }
    ...
    private static void checkDeprecationWarning(GradleVersion targetGradleVersion) {
        if (targetGradleVersion.compareTo(MINIMUM_SUPPORTED_GRADLE_VERSION) < 0) {
            DeprecationLogger.deprecate(String.format(
                            "The version of Gradle you are using (%s) is deprecated with TestKit. TestKit will only"
                                    + " support the last 5 major versions in future.",
    ...
    private GradleConnector buildConnector(
        ...
        gradleConnector.daemonBaseDir(new File(gradleUserHome, daemonDirName));
    ...
    public String toString() {
        return String.format("NamedToolingApiGradleExecutor(%s)", daemonDirName);
    }
}
